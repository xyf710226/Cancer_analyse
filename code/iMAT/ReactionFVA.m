...
function [React_High,React_Low] = ReactionFVA(HighSet,LowSet,Network,MaxFlux)
    ...
    for i=1:reactionNum
		...
		if ~isempty(temp)
			...
			React_High.exitFlag(i,1) = exitFlag;
%			React_High.outPut{i,1} = outPut;
			if exitFlag < 1
				React_High.objective(i,1) = 0;
			else 
				React_High.objective(i,1) = fval;
			end        
	...
	React_Low.objective = zeros(reactionNum,1);  
	React_Low.exitFlag = zeros(reactionNum,1);
	React_Low.outPut = cell(reactionNum,1);
    ...
    for i = 1:reactionNum
		...
		if ~isempty(temp)
		   %% Solve the MILP problem when reaction i belongs to Down-changed Reaction Set
			...
			React_Low.exitFlag(i,1) = exitFlag;
			React_Low.outPut{i,1} = outPut;
			if exitFlag < 1
				React_Low.objective(i,1) = 0;
			else 
				React_Low.objective(i,1) = fval;
			end        
			clear Aeq Beq Aineq Bineq ub lb;
			continue;
		end
